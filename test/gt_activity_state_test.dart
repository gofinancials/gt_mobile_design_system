...
void main() {
  ...
  group('GtActivityState Unit Tests', () {
    ...
    test('registerActivity throttles frequent activity registrations', () async {
      activityState.startTracking(onInactivity: (_) {});
    ...
    test('Inactivity timeout triggers onInactivityDetected callback with RouteSettings', () async {
      RouteSettings? capturedSettings;
      bool callbackFired = false;

      activityState.startTracking(
        duration: 150.milliseconds,
        initialRouteSettings: const RouteSettings(name: '/transfer', arguments: {'amount': 5000}),
    ...
    test('AppLifecycleState.paused stops debouncer and resumed evaluates elapsed time', () async {
      RouteSettings? capturedSettings;
      bool callbackFired = false;
  ...
  group('GtActivityRouteObserver Tests', () {
    ...
    testWidgets('didPush registers route settings with GtActivityState', (tester) async {
      ...
      expect((activityState.currentRouteSettings?.arguments as Map)['id'], equals(123));
  ...
  group('GtBaseWidget Activity Interaction Tests', () {
    ...
    testWidgets('Pointer touches inside GtBaseWidget register user activity', (tester) async {
      await tester.pumpWidget(
        ChangeNotifierProvider<GtActivityState>.value(
          ...
          child: const MaterialApp(
            home: GtBaseWidget(
              child: Scaffold(
                body: Center(child: Text('Interactive Content')),
              ),
            ),
          ),
        ),
    ...
    testWidgets('Scroll events inside GtBaseWidget register user activity', (tester) async {
      await tester.pumpWidget(
        ChangeNotifierProvider<GtActivityState>.value(
          value: activityState,
          child: MaterialApp(
